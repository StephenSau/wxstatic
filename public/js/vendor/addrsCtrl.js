...
var addrsCtrl = {
    ...
    addressSelectAction: function (params) {
        ...
        addrsCtrl.clearSelectbox(params.selectObj);
        if (params.value !== "请选择" && params.value !== "") {
            items = addrsCtrl.qryPlace(params.value, params.isCity);
            length = items.length;
            if (length !== 0) {
        ...
        if (params.resetObj !== undefined) {
            addrsCtrl.clearSelectbox(params.resetObj);
            firstOption = document.createElement('option');
            firstOption.appendChild(document.createTextNode("请选择"));
            firstOption.setAttribute("value", "");
    ...
    qryPlace: function (parentCode, isCity) {
        var i = 0,
            j = 0,
            k = 0,
            provinceLength = ycs_sys_addrs.length,
            cityLength = 0,
            districtLength = 0,
            result = [];
        if (parentCode !== undefined) {
            if (isCity) {
                for (i = 0; i < provinceLength; i += 1) {
                    if (parseInt(parentCode, 10) !== ycs_sys_addrs[i].c) {
                        continue;
                    }
                    for (j = 0, cityLength = ycs_sys_addrs[i].s.length; j < cityLength; j += 1) {
                        result.push({
                            code: ycs_sys_addrs[i].s[j].c,
                            name: ycs_sys_addrs[i].s[j].n
                        });
                    }
                }
            } else {
                for (i = 0; i < provinceLength; i += 1) {
                    if (parseInt(parentCode.substr(0, 2).concat("0000"), 10) !== ycs_sys_addrs[i].c) {
                        continue;
                    }
                    for (j = 0, cityLength = ycs_sys_addrs[i].s.length; j < cityLength; j += 1) {
                        if (parseInt(parentCode, 10) !== ycs_sys_addrs[i].s[j].c) {
                            continue;
                        }
                        for (k = 0, districtLength =  ycs_sys_addrs[i].s[j].s.length; k < districtLength; k += 1) {
                            result.push({
                                code: ycs_sys_addrs[i].s[j].s[k].c,
                                name: ycs_sys_addrs[i].s[j].s[k].n
                            });
                        }
                    }
                }
            ...
            for (i = 0; i < provinceLength; i += 1) {
                result.push({
                    code: ycs_sys_addrs[i].c,
                    name: ycs_sys_addrs[i].n
                });
            }
        }
        return result;
    ...
    qryName: function (code) {
        ...
        if (code % 10000 === 0) {
            tempList = dichotomy(code, 0, ycs_sys_addrs.length - 1, ycs_sys_addrs);
            if (tempList) {
                return tempList.n;
            }

        } else if (code % 100 === 0) {
            tempList = dichotomy(Math.floor(code / 10000) * 10000, 0, ycs_sys_addrs.length - 1, ycs_sys_addrs);
            if (tempList) {
